...
option = st.selectbox('Select Your State', ('New York','California','New Jersey', 'Texas', 'Washington'))
...
if option == 'New York':
    ...
    with _left:
        st.image("./newyork.gif",width =700)
    details(New_York)
    ...
    _left, mid, _right = st.columns(3)
    with _left:
        st.image("./california.gif",width =700)
    details(California)

elif option == 'New Jersey':
    image = Image.open('./newjersey.jpg')
    st.image(image, use_column_width=True)
    details(New_Jersey)
    ...
    _left, mid, _right = st.columns(3)
    with _left:
        st.image("./texas.gif",width =700)
    details(Texas)

elif option == 'Washington':
    _left, mid, _right = st.columns(3)
    with _left:
        st.image("./washignton.gif",width =700)
    details(Washington)
